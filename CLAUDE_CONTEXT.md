...
| 🟡 | — | **GitHub Releases v1.297–v1.300** | Create formal releases at github.com/PatriotsRV/rv-dashboard/releases/new — v1.297 (photo library null crash fix), v1.298 (video upload size error), v1.299 (editField apostrophe crash), v1.300 (Kevin McHenry Sr. Manager) | ⏳ Roland action |
| 🟡 | — | **GitHub Release v1.301** | Create formal release at github.com/PatriotsRV/rv-dashboard/releases/new — tag `v1.301` — checkin.html v1.28 Supabase auth fix | ⏳ Roland action |
| ✅ | — | **Fix checkin.html Supabase auth for time_logs** | Fixed in v1.28/v1.301. `signInWithIdToken()` now called on sign-in; `getSession()` restores session on reload; `onAuthStateChange` keeps session in sync; `clockIn()` guards online insert. **Roland action: go to Supabase → Auth → Policies → time_logs and remove the two anon INSERT/UPDATE policies — they are no longer needed.** | ✅ Done v1.28 |
| 🟠 | — | **Remove anon policies on time_logs in Supabase** | Now that checkin.html v1.28 properly establishes an authenticated session, the temporary anon INSERT/UPDATE policies on `time_logs` can be safely deleted. Go to Supabase dashboard → Table Editor → time_logs → RLS Policies → delete "Anon can insert time_logs" and "Anon can update time_logs". | ⏳ Roland action |
...
### checkin.html — Auth Fixed in v1.28 (Session 29)
- **Root cause:** `handleSignIn()` decoded Google JWT locally but never called `signInWithIdToken()` — every DB operation (clock-in insert, clock-out update) fired as anon role, not authenticated.
- **Fix (v1.28):** `generateNonce()` helper generates hex SHA-256 nonce. `hashedNonce` passed to GIS `initialize()` + `params` (Chrome 145 compat). `signInWithIdToken()` called with `rawNonce` in `handleSignIn()`. `getSB()` now uses `persistSession: true, autoRefreshToken: true`. `getSession()` called on `DOMContentLoaded` to restore 30-day session. Offline identity fallback gated by `!isOnline`. `onAuthStateChange` listener. `clockIn()` guards with `!supabaseSession`. `signOut()` calls `auth.signOut()`.
- **Roland must remove anon INSERT/UPDATE policies on `time_logs`** — go to Supabase → Table Editor → time_logs → RLS Policies → delete "Anon can insert time_logs" + "Anon can update time_logs".
- checkin.html uses default Supabase storage key (not `prvs_supabase_auth` like index.html) — separate session, intentional.

### Supabase Pro Security Cleanup (Session 28)
- Removed all `Anon can...` write policies from: `repair_orders`, `parts`, `users`, `user_roles`, `notes`, `audit_log`, `cashiered`, `config`, `insurance_scans`. Kept anon policies on `time_logs` (see above — now fixed in v1.28).
- Fixed `has_role(text)` and `is_silo_manager(text)` mutable search_path via `ALTER FUNCTION ... SET search_path = public`.
- Remaining ~31 Security Advisor warnings are all `authenticated_full_access` (intentional — all users are PRVS staff) + solar tables (intentional).
- `user_roles` INSERT is still open to any authenticated user — low risk but could allow privilege escalation via raw API. Future improvement: restrict to `has_role('Admin')`.
