...
|---|---|---|
| `index.html` | **v1.294** | Main dashboard — ROs, time tracking, parts, calendar, audit log, parts request system with photo attachments, photo lightbox viewer, email photos to customer, Spanish language toggle, video upload support, duplicate RO manager (Admin), Date RV Arrived on Lot save fix, four-state RO-level parts chip system (Sourcing/Outstanding/Received/Estimate), For Estimate Only toggle, Manager role expanded, Wholesale/Retail price columns in Manage Parts table, Freight Charge label (was Core Charge), Sourcing added to individual part row status dropdown, Supplier Contact section moved to top of Add/Edit Part form, Kenect messaging integration (💬 button on RO cards, conversation modal, send/review), 📍 Parking Spot field (R/B/W/F lot positions), 🖨️ Dual-sticker QR Print Sheet (3"×3" windshield + 1"×1" key tag), **QR codes link to main dashboard RO tile with deep-link highlight (v1.294)** |
...
| 2026-03-27 | 25 | v1.294 — QR scan destination changed from checkin.html to main dashboard RO tile: `generateQRCode()`, QR modal, and `printQRLabel()` all updated to `/?ro={roId}`; `handleDeepLink()` added with `_deepLinkRoId` one-shot flag; fires at end of `renderBoard()`, scrolls to matching tile and pulses 3× blue highlight ring; `openCheckIn()` (🚪 Tech Check In) unchanged. Kenect Zapier discussion: reviewed Kenect-provided PDF — Zapier integration is outbound-only for messaging (Send Message, Send Review Request, Push Lead as Actions; only Lead Push as Trigger — no inbound message trigger). Advised Roland to push for direct API access since conversation thread view requires inbound message querying. End of Session Checklist run; backup.sh executed; pushed. |
