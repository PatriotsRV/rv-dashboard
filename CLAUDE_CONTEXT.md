...
| 🟠 | GH#23 | **Morning Manager Report — data quality banner fix** | Data quality banner ($0.00 warning) not firing for Roland's sr_manager report — sr_manager multi-silo work list lookup still not resolving correctly. `send-manager-report` v1.5 deployed and sending emails, but sr_manager first-silo unassigned work list items need fresh investigation. Deferred from Session 44. | 🔄 In Progress — deferred |
...
| 🟡 | — | **GitHub Release v1.402/v1.403/v1.404** | v1.402: Warranty RO type. v1.403: Slack integration (5 triggers + slack-notify Edge Function). v1.404: GH#23 Morning Manager Report (send-manager-report v1.5). github.com/PatriotsRV/rv-dashboard/releases/new — tag `v1.402`, `v1.403`, `v1.404` | ⏳ Roland action |
...
| `supabase/functions/send-manager-report/index.ts` | **v1.5** | Edge Function — GH#23 Morning Manager Report. Per-silo personalized email to all managers (sr_manager, manager, parts_manager). Section 1: Manager's Work List with cumulative dollar total. Section 2: RVs Waiting for their silo sorted by days on lot → urgency → RO type. Section 3: Key Flags (Critical, overdue >60 days, blocking parts). Red "YOUR WORK LIST IS EMPTY" banner per silo. Yellow "YOUR WORK LIST APPEARS STALE" banner (3+ days no update). Scheduled Mon–Fri 8 AM CDT via pg_cron job ID 5. **Deployed and confirmed sending emails 2026-04-13.** Commits: f875bc4 (v1.0), e369936 (v1.1), 0d8d462 (v1.2), 1055b82 (v1.2b), 665806b (v1.3), efe60bb (v1.3b), f28a2d7 (v1.3c), c7d2770 (v1.4), 4c3e9a7 (v1.5). |
...
### Morning Manager Report — send-manager-report v1.5 (GH#23, Session 44)
- `send-manager-report` Edge Function deployed to Supabase, pg_cron job ID 5 (Mon–Fri 8 AM CDT = 13:00 UTC).
- Per-silo personalized email to all managers. Section 1: Work List + dollar total. Section 2: RVs Waiting sorted by days on lot. Section 3: Key Flags (Critical, >60d, blocking parts).
...
- Commits: f875bc4 (v1.0), e369936 (v1.1), 0d8d462 (v1.2), 1055b82 (v1.2b), 665806b (v1.3), efe60bb (v1.3b), f28a2d7 (v1.3c), c7d2770 (v1.4), 4c3e9a7 (v1.5).
