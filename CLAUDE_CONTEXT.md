...
| 🟡 | GH#18 | **Parts Ordered Email Notification** | **Session 32:** Dashboard v1.304 — `requested_by_email` captured on `submitPartsRequest`, stored to DB + local data. `notifyPartsRequester()` manual trigger button in Parts Status modal. `notifyPartsEtaUpdate()` auto-fires on ETA set/change in `savePartForm`. `send-quote-email` v1.5 adds `parts_ordered` + `parts_eta_update` email types. `send-parts-report` Edge Function deployed (4-section HTML email: open requests, ordered/in-transit, overdue, received 24h). `parts-report.yml` cron live (Mon–Fri 8 AM + 3 PM CDT). Migration ran ✅. CLI deploy ✅. First report delivered ✅. **Session 37: `ro_id` FK bug fixed in sections 2/3/4. Redeployed ✅.** **Session 41: `send-parts-report` v1.3 — contextual action prompts above each of 4 data sections; morning email "Good morning Bobby" banner with numbered priority order; afternoon email "Before You Leave Today" 6-item EOD checklist; conditional section prompts (empty = nothing to do, data = specific actions); plain text version updated. ⚠️ Roland must deploy: `supabase functions deploy send-parts-report`.** Pending: test Notify Requester button + ETA email. | 🔄 Testing notifications |
...
| `supabase/functions/send-parts-report/index.ts` | **v1.3** | Edge Function — Parts email report — contextual action prompts + EOD checklist. 4 data sections (open requests, ordered/in-transit, overdue, received 24h) each with contextual action prompts above (empty = nothing to do, data = specific actions). Morning email: "Good morning Bobby" banner with numbered priority order. Afternoon email: "Before You Leave Today" 6-item EOD checklist. Plain text version updated to match. Queries DB via service role, emails all sr_managers + managers + parts_managers. **v1.2: S7 CORS. v1.1: ro_id FK fix. ⚠️ Requires CLI deploy: `supabase functions deploy send-parts-report`** |
...
### send-parts-report v1.3 Deployment (Session 41)
- `send-parts-report v1.3` is committed but **not yet deployed** — Roland must run `supabase functions deploy send-parts-report` from his machine to activate.
