...
| 🟠 | — | **Fix checkin.html Supabase auth for time_logs** | checkin.html clock-in inserts hit `time_logs` as anon role (Supabase session not established before insert). Broke when anon policies were removed during Supabase Pro security cleanup. Temporarily restored anon INSERT/UPDATE on time_logs as workaround. Proper fix: ensure `signInWithIdToken` completes and session is confirmed before allowing clock-in attempt. | ⏳ Open |
...
### checkin.html — Anon Role Dependency for time_logs (Session 28)
- `checkin.html` clock-in inserts hit `time_logs` using the Supabase **anon** role, not `authenticated`. This was discovered when we removed all anon write policies during the Supabase Pro security cleanup — clock-in immediately broke with "new row violates row-level security policy for table time_logs".
- **Root cause:** `checkin.html`'s Supabase session may not be fully established before the clock-in insert fires, or the Google Sign-In flow on iPhone does not complete `signInWithIdToken` in time.
- **Temporary workaround:** Restored `Anon can insert time_logs` and `Anon can update time_logs` policies via SQL.
- **Proper fix needed:** Ensure `signInWithIdToken` completes and `supabaseSession` is confirmed non-null before the clock-in button submits. Gate the insert on session existence, show a "Signing in..." state if needed.

### Supabase Pro Security Cleanup (Session 28)
- Removed all `Anon can...` write policies from: `repair_orders`, `parts`, `users`, `user_roles`, `notes`, `audit_log`, `cashiered`, `config`, `insurance_scans`. Kept anon policies on `time_logs` (see above).
