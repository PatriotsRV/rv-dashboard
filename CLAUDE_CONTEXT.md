...
| 🟠 | GH#29b | **Clean up duplicate/test ROs in Supabase** | Follow-up to GH#29 code fix. Accumulated junk rows in `repair_orders` from a month of testing are the root cause of the user-visible symptom (wrong-row writes). **Approach:** (1) Run detection SQL in Supabase SQL Editor to surface dupes grouped by `(customer_name, date_received)` + test-signature names ("test", "tester", "fake", etc.). (2) Triage with Roland — some `(name, date)` collisions may be legit repeat customers. (3) For confirmed customer dupes use the existing 🔗 Merge Dupes admin button (reassigns FKs on notes/parts/time_logs/insurance_scans/audit_log, merges photo_library, copies rvPhotoUrl if master missing, deletes duplicates). For pure test junk, direct DELETE in SQL Editor (Admin RLS). Do NOT delete anything with real customer data without Roland explicitly confirming the row. **Session 51 scan found only 1 dupe group (Roman / 2017 Thor Aria / 2026-04-18) created during today's check-in testing — keeper: `1293920d` (PRVS-5493-808C-2, Awaiting parts, 5 audit entries); soft-delete target: `bc206e95` (PRVS-5493-808C, Not On Lot, 1 note). Awaiting GH#30 Delete RO feature to handle via soft-delete + 1-week auto-scrub.** | 🔄 Review complete — cleanup pending GH#30 |
