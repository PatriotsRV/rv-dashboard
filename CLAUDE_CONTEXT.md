...
| `worklist-report.html` | **v1.2** | Admin-only Active Work List Report — all managers' work lists + time logs per RO. Manager accordion sections (Sr Managers grouped by silo). Condensed RO rows with KPI chips (Days on Lot color-coded, Dollar Value). Expandable time log detail per RO. Staff Status tiles (red/green clock-in grid) **with clickable tech tile expand — daily time log detail per tech (sessions, gaps, duration, ACTIVE badge, summary line)**. Auto-refresh every 3 min. Google SSO auth gate. **S2 Phase 3: RBAC migration — loadUserRoles, role-based isAdmin, _allStaff lookups, hardcoded email arrays removed.** |
...
*Last updated: 2026-04-18 — Session 51 — v1.409*
