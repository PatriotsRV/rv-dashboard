...
| 🟠 | GH#23 | **Morning Manager Report — data quality banner fix** | Data quality banner ($0.00 warning) not firing for Roland's sr_manager report — sr_manager multi-silo work list lookup still not resolving correctly. `send-manager-report` v1.8 deployed (Supabase version 13). **Session 45 improvements:** v1.7 changed filter from ≥60 to ≥30 days on lot; v1.8 added red PARTS HOLD badge on individual RO line items + named ROs in Key Flags section. Remaining: sr_manager data quality banner still needs service_silo mapping in staff table. | 🔄 In Progress — deferred |
...
| 🟡 | — | **GitHub Release v1.402/v1.403/v1.404** | v1.402: Warranty RO type. v1.403: Slack integration (5 triggers + slack-notify Edge Function). v1.404: GH#23 Morning Manager Report (send-manager-report v1.8). github.com/PatriotsRV/rv-dashboard/releases/new — tag `v1.402`, `v1.403`, `v1.404` | ⏳ Roland action |
