...
- **GH#35 — Offboard nik@patriotsrvservices.com (TERMINATED)** (Session 54) — Copy-paste SQL delivered in chat: (1) UPDATE staff active=false + clear SMS, (2) DELETE user_roles, (3) DELETE users. Plus Supabase Auth (Sign out + Delete user) + Google Workspace suspend + GitHub org remove + Slack deactivate + shared-secret rotation. Preserves time_logs/audit_log/RO assignment history. ⏳ Roland action — execute when ready
...
## ✅ Recently Completed
- ✅ **Staff Offboarding Playbook — Nik Polizzo terminated (2026-04-22, Session 54)** — Full offboarding SQL + dashboard checklist delivered to Roland for `nik@patriotsrvservices.com`. DB side: `UPDATE staff SET active=false + clear SMS`, `DELETE FROM user_roles`, `DELETE FROM users`. Supabase Auth: Sign out user + Delete user. Google Workspace + GitHub org + Slack + shared-secret rotation handled externally by Roland. Preserves `time_logs`, `audit_log`, `repair_orders.technician_assigned` for audit/payroll/historical integrity. Tracked as GH#35. Reusable "Staff Offboarding Pattern" gotcha added to CLAUDE_CONTEXT.md for future terminations.
...
*Last updated: 2026-04-22 — Session 54 — index v1.409 · worklist-report v1.9 · checkin.html v1.30. Session delivered: staff offboarding playbook for terminated employee Nik Polizzo (`nik@patriotsrvservices.com`) — inline SQL + Supabase Auth dashboard steps + external-services checklist handed to Roland as GH#35 Roland-action. Reusable "Staff Offboarding Pattern" Known Issues entry added to CLAUDE_CONTEXT.md for future terminations. No version bump; no code changes.*
